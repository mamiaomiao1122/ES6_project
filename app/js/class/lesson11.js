...
//建议：优先使用map；如果对数据要求较高（唯一性）考虑使用set；放弃使用object和数组做存储
{
	...
	console.log(array_exist);//返回的值:当前的对象  {t: 1}
	...
	// 删
	map.delete('t');
	let index = array.findIndex(item=>item.t);//位置的索引
...

{
	...
	// 增
	set.add({t:1}); //注意参数是对象，和map有区别
	array.push({t:1});
	console.log('set -array',set,array);

	// 查
	let set_exist = set.has({t:1});//注意参数是对象，和map有区别；；被保存后再查地址
	let array_exist = array.find(item=>item.t);
	...
	array.forEach(item=>item.t?item.t=2:"");
	console.log('set-array',set,array);  //{"t" => 2} [{t:2}]
